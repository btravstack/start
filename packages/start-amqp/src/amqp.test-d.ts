// Type-level tests (checked by `tsc`, no runtime). Same two guarantees as the HTTP host, at the
// consume() mount: the disposition map is TOTAL over the handler's inferred error union, and a
...
import { amqp, createConsumer } from "./index.js";
...
const consumer = createConsumer<AppSvc>()(reqLayer);
