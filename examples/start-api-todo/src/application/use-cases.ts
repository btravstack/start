// Application — the three use cases, in the three demesne shapes, to show they all bind the same
// way at the transport edge (`handler.use`):
//   • GetTodo    — a `Service` class:      ctx.get(GetTodo).execute(id)
//   • CreateTodo — an `inject` closure:    ctx.get(CreateTodo)(input)
//   • ListTodos  — an `inject` closure:    ctx.get(ListTodos)()
// Each is constructor-injected from its ports; none imports the transport.
